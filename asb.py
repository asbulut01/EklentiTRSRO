	...
	if player and lstLeaders_exist(player) or t == 100:
		...
		if msg == "START":
			...
			if msg == "R ":
				...
				try:
					# split and parse movement radius
					radius = int(float(msg[1:].split()[0]))
					# to absolute
					radius = (radius if radius > 0 else radius*-1)
					set_training_radius(radius)
					log("Plugin: Training radius set to "+str(radius)+" m.")
			...
			# Live PT trigger: ignore extra text and use first nearby NPC UID
			# Get NPCs and extract first UID as integer
			npcs = get_npcs()
			if not npcs:
				log('Plugin: No NPCs found to perform PT command')
			else:
				uid = None
				if isinstance(npcs, dict):
					...
					try:
						...
						uid = None
				if not uid:
					log('Plugin: No valid NPC UID found to perform PT command')
				else:
					# Pack UID as little-endian 4 bytes and send packets
					data = struct.pack('<I', uid)
					log("Plugin: Sending PT packets for UID %08X" % uid)
					inject_joymax(0x7045, data, False)
					Timer(1.0, inject_joymax, (0x704B, data, False)).start()
					Timer(2.0, inject_joymax, (0x705A, data + b'\x03\x00', False)).start()
...
def scan_npcs():
    """Scan nearby NPCs and return a list of 4-byte hex UID strings for NPCs named 'Dimensional Gate'.
    ...
    try:
        ...
        if isinstance(npcs, dict):
            for uid, npc in npcs.items():
                ...
                if isinstance(npc, dict):
                    ...
                    name = str(npc).strip().lower()
                # only collect Dimensional Gate
                if name == 'dimensional gate':
                    hex_uid = format_uid_bytes(uid)
                    log(f"TR_ASB: Matched Dimensional Gate - {uid} UID_HEX: {hex_uid}")
                    hex_list.append(hex_uid)
            if hex_list:
                log(f"TR_ASB: {len(hex_list)} Dimensional Gate NPC(s) found.")
            else:
                log("TR_ASB: No 'Dimensional Gate' NPC found nearby.")
            return hex_list
        ...
        for npc in npcs:
            ...
            if isinstance(npc, dict):
                ...
                name = str(npc).strip().lower()
            try:
                if name == 'dimensional gate' and uid is not None:
                    hex_uid = format_uid_bytes(uid)
                    log(f"TR_ASB: Matched Dimensional Gate - {uid} UID_HEX: {hex_uid}")
                    hex_list.append(hex_uid)
            except Exception:
...
def format_npc_pack(npc):
    ...
    if not items:
        return 'Paket yok veya eri�ilemez'
    ...
    return ', '.join(parts) if parts else 'Bo� paket'

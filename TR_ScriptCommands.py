...
def InjectPackets():
	...
	if QtBind.isChecked(gui,cbxShowPackets):
		log("Eklenti: Enjekte edildi (İşlem Kodu). 0x" + '{:02X}'.format(opcode) + " (Data) "+ ("None" if not data else ' '.join('{:02X}'.format(x) for x in data)))
...
def handle_silkroad(opcode, data):
	...
	if BtnStart:
		if opcode == 0x7045:
			...
			if QtBind.isChecked(gui,cbxShowPackets):
				log("Eklenti: Kayıtlı (İşlem Kodu) 0x" + '{:02X}'.format(opcode) + " (Data) "+ ("None" if not data else ' '.join('{:02X}'.format(x) for x in data)))
		if Recording == True:
			if opcode != 0x7045:
				RecordedPackets.append("0x" + '{:02X}'.format(opcode) + ":" + ' '.join('{:02X}'.format(x) for x in data))
				if QtBind.isChecked(gui,cbxShowPackets):

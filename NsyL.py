...
pName = 'NsyL'
pVersion = '2.1'
pUrl = 'https://raw.githubusercontent.com/hakankahya48/EklentiTRSRO/main/NsyL.py'
...
btnAddChannel = QtBind.createButton(gui,'btnAddChannel_clicked',"   Ekle   ",86,22)
btnRemChannel = QtBind.createButton(gui,'btnRemChannel_clicked',"     Sil     ",45,137)

# Discord options
QtBind.createLabel(gui,"Token :",6,165)
tbxToken = QtBind.createLineEdit(gui,"",43,163,119,19)

cbxAddTimestamp = QtBind.createCheckBox(gui,'cbxDoNothing','Zaman Damgası Ekle',6,185)

# Separator line
QtBind.createLineEdit(gui,"",169,10,1,262)

# Triggers
QtBind.createLabel(gui,"Bildirimi göndermek için Discord kanalını seçin (Filtreler normal ifade kullanıyor! )",175,10)
QtBind.createLabel(gui,"Edited by ' NsyL#1634 ' , ' Xenopbia  ' and ' hakankahya ' ",420,270)
btnSaveConfig = QtBind.createButton(gui,'saveConfigs',"Değişiklikleri Kaydet",615,4)
...
_y+=20
QtBind.createLabel(gui,'Oyunla Bağlantısı Kesildi',_x+_cmbxWidth+4,_y+3)
cmbxEvtChar_disconnected = QtBind.createCombobox(gui,_x,_y,_cmbxWidth,_Height)
_y+=20

# uniques
_y += 5
QtBind.createLabel(gui,'Unique Doğdu',_x+_cmbxWidth+4,_y+3)
...
_y+=20
QtBind.createLabel(gui_,'Bildirim',_x+_cmbxWidth+4,_y+3)
...
_x += _cmbxWidth * 2 + 10
_y = 7
QtBind.createLabel(gui_,'Öğe alındı (vSRO)',_x+_cmbxWidth+4,_y+3)
...
_y+=20
QtBind.createLabel(gui_,'Öğe (Nadir) alındı',_x+_cmbxWidth+4,_y+3)
cmbxEvtPick_rare = QtBind.createCombobox(gui_,_x,_y,_cmbxWidth,_Height)
_y+=20
QtBind.createLabel(gui_,'Öğe (Donatılabilir) alındı',_x+_cmbxWidth+4,_y+3)
...
def loadDefaultConfig():
	...
	QtBind.setText(gui,tbxEvtMessage_uniqueSpawn_filter," İsme göre filtrele")
	QtBind.setChecked(gui,cbxEvtMessage_uniqueKilled_filter,False)
	QtBind.setText(gui,tbxEvtMessage_uniqueKilled_filter," İsme göre filtrele")
	...
	QtBind.setText(gui_,tbxEvtMessage_global_filter," Mesaja göre filtrele")
	QtBind.setChecked(gui_,cbxEvtMessage_notice_filter,False)
	QtBind.setText(gui_,tbxEvtMessage_notice_filter," Mesaja göre filtrele")

	QtBind.setChecked(gui_,cbxEvtPick_name_filter,False)
	QtBind.setText(gui_,tbxEvtPick_name_filter," İsme göre filtrele")
	QtBind.setChecked(gui_,cbxEvtPick_servername_filter,False)
	QtBind.setText(gui_,tbxEvtPick_servername_filter," Sunucu adına göre filtrele")
...
def btnAddChannel_clicked():
	if character_data:
		...
		if channel_id.isnumeric():
			...
			log('Plugin: Hata, Discord Kanal Kimliği bir numara olmalı!')
...
def btnRemChannel_clicked():
	if character_data:
		...
		if channelItem:
			...
			log('Plugin: Kanal kaldırıldı > ['+channelItem+']')
		...
		with urllib.request.urlopen(req,timeout=5) as f:
			try:
				...
				if msg == 'true' or msg == 'success':
					log("Plugin: Discord'a bildirim gönderildi!")
				else:
					log("Plugin: Bildirim başarısız oldu! > ["+msg+"]")
			except Exception as ex2:
				log("Plugin: Sunucudan yanıt okunurken hata oluştu ["+str(ex2)+"]")
	except Exception as ex:
		log("Plugin: Bildirmek için ["+str(ex) +"] yüklenirken hata oluştu.")
			...
			try:
				...
				if resp:
					if resp == 'true' or resp == 'success':
						...
						log("Plugin: Getirme başarısız oldu. ["+resp['message']+"]")
			except Exception as ex2:
				log("Plugin: Sunucudan yanıt okunurken hata oluştu ["+str(ex2)+"]")
	except Exception as ex:
		log("Plugin: Url yüklenirken hata oluştu ["+str(ex)+"] ")
		...
		if path.is_file() and path.name.endswith(".py") and path.name != plugin_name:
			try:
				...
				if hasattr(plugin,'handle_chat'):
					...
					log('Plugin: Yüklenen discord işleyicisi '+path.name)
			except Exception as ex:
				log('Plugin: Eklenti Yükleme Hatası '+path.name+' '+str(ex))
	return handlers
...
def joined_game():
	loadConfigs()
	Notify(QtBind.text(gui,cmbxEvtChar_joined),"|`"+character_data['name']+"`| - Oyuna bağlandı.",colour=0xFF00FF)
...
def disconnected():
	...
	if isOnline:
		...
		if channel_id:
			Notify(channel_id,"|`"+character_data['name']+"`| Bağlantınız kesildi.",colour=0x00DDFF)
...
def handle_chat(t,player,msg):
	...
	# Check message type
	if t == 1:
		Notify(QtBind.text(gui_,cmbxEvtMessage_all),"|`"+character_data['name']+"`| - [Genel] `"+player+"`: "+msg,colour=0x00FF00)
	elif t == 2:
		Notify(QtBind.text(gui_,cmbxEvtMessage_private),"|`"+character_data['name']+"`| - [**Özel**] `"+player+"`: "+msg,colour=0x00FFFF)
	elif t == 9:
		Notify(QtBind.text(gui_,cmbxEvtMessage_stall),"|`"+character_data['name']+"`| - [**Tezgah**] `"+player+"`: "+msg,colour=0x00FF00)
	elif t == 4:
		Notify(QtBind.text(gui_,cmbxEvtMessage_party),"|`"+character_data['name']+"`| - "+"[**Parti**] `"+player+"`: "+msg,colour=0x0000FF)
	elif t == 16:
		Notify(QtBind.text(gui_,cmbxEvtMessage_academy),"|`"+character_data['name']+"`| - "+"[**Akademi**] `"+player+"`: "+msg,colour=0x00DDFF)
	elif t == 5:
		Notify(QtBind.text(gui_,cmbxEvtMessage_guild),"`"+player+"`: "+msg,colour=0xFF9900)
	elif t == 11:
		Notify(QtBind.text(gui_,cmbxEvtMessage_union),"[**Birlik**] `"+player+"`: "+msg,colour=0x00FF00)
	elif t == 6:
		if QtBind.isChecked(gui_,cbxEvtMessage_global_filter):
			...
			if searchMessage:
				try:
					...
					log("Plugin: Regex'te hata oluştu ["+str(ex)+"]")
		else:
			Notify(QtBind.text(gui_,cmbxEvtMessage_global),"   `"+player+"`: **"+msg+"**",colour=0xFFFF00)
	elif t == 7:
		if QtBind.isChecked(gui_,cbxEvtMessage_notice_filter):
			...
			if searchMessage:
				try:
					if re.search(searchMessage,msg):
						Notify(QtBind.text(gui_,cmbxEvtMessage_notice),"[**Simya**] : "+msg,colour=0xFF00FF)
				except Exception as ex:
					log("Plugin: Regex'te hata oluştu ["+str(ex)+"]")
		else:
			Notify(QtBind.text(gui_,cmbxEvtMessage_notice),"[**Simya**] : "+msg,colour=0xFF00FF)
...
def handle_event(t, data):
	# Filter events
	if t == 9:
		Notify(QtBind.text(gui,cmbxEvtNear_gm),"|`"+character_data['name']+"`| - **GameMaster** `"+data+"` sana yakın!",CreateInfo("position",get_position()),colour=0xFF5722)
	elif t == 0:
		Notify(QtBind.text(gui,cmbxEvtNear_unique),"|`"+character_data['name']+"`| - **"+data+"** Unique sana yakın!",CreateInfo("position",get_position()),colour=0xFF5722)
	elif t == 1:
		Notify(QtBind.text(gui,cmbxEvtNear_hunter),"|`"+character_data['name']+"`| - **Avcı / Tüccar** `"+data+"` sana yakın!",CreateInfo("position",get_position()),colour=0xFF5722)
	elif t == 2:
		Notify(QtBind.text(gui,cmbxEvtNear_thief),"|`"+character_data['name']+"`| - **Hırsız** `"+data+"` sana yakın!",CreateInfo("position",get_position()),colour=0xFF5722)
	elif t == 4:
		Notify(QtBind.text(gui,cmbxEvtChar_attacked),"|`"+character_data['name']+"`| - `"+data+"` sana saldırıyor!",colour=0xFF5722)
	elif t == 7:
		Notify(QtBind.text(gui,cmbxEvtChar_died),"|`"+character_data['name']+"`| - Öldün.",CreateInfo("position",get_position()),colour=0xFF5722)
	elif t == 3:
		pet = get_pets()[data]
		Notify(QtBind.text(gui,cmbxEvtPet_died),"|`"+character_data['name']+"`| - Evcil hayvanınız `"+(pet['type'].title())+"` öldü!",colour=0xFF5722)
	elif t == 5:
		...
		if channel_id:
			...
			genre = getGenreText(item['servername'])
			sox = getSoXText(item['servername'],item['level'])
			Notify(channel_id,"|`"+character_data['name']+"`| - **Öğe(Nadir)** Alındı > ***"+item['name']+(' '+race if race else '')+(' '+genre if genre else '')+(' '+sox if sox else '')+"***",colour=0xFF9900)
	elif t == 6:
		...
		if channel_id:
			...
			genre = getGenreText(item['servername'])
			sox = getSoXText(item['servername'],item['level'])
			Notify(channel_id,"|`"+character_data['name']+"`| - **Öğe(Donatılabilir)** Alındı > ***"+item['name']+(' '+race if race else '')+(' '+genre if genre else '')+(' '+sox if sox else '')+"***",colour=0xFF9900)
	elif t == 8:
		Notify(QtBind.text(gui,cmbxEvtBot_alchemy),"|`"+character_data['name']+"`| - **Simya** tamamlandı",colour=0x0000FF)
...
def handle_joymax(opcode, data):
		...
		if updateType == 5:
					...
					if searchName:
						try:
							if re.search(searchName,uniqueName):
								Notify(channel_id,"**"+uniqueName+"** ortaya çıktı!",colour=0x9C27B0)
						except Exception as ex:
							log("Plugin: Regex'te hata oluştu. ["+str(ex)+"]")
				else:
					Notify(channel_id,"**"+uniqueName+"** ortaya çıktı!",colour=0x9C27B0)
			...
			if channel_id:
				...
				if QtBind.isChecked(gui,cbxEvtMessage_uniqueKilled_filter):
					...
					if searchName:
						try:
							if re.search(searchName,uniqueName):
								Notify(channel_id,"**"+uniqueName+"** `"+killerName+"` tarafından öldürüldü!",colour=0x9C27B0)
						except Exception as ex:
							log("Plugin: Regex'te hata oluştu. ["+str(ex)+"]")
				else:
					Notify(channel_id,"**"+uniqueName+"** `"+killerName+"` tarafından öldürüldü!",colour=0x9C27B0)
			...
			if jobType == 1:
				...
				if channel_id:
					progressType = data[3]
					if progressType == 0:
						Notify(channel_id,"[**Konsinye**] Avcı ticareti 10 dakikada başlayacak!",colour=0x0000FF)
					elif progressType == 1:
						Notify(channel_id,"[**Konsinye**] Avcı ticareti başladı! ("+getConsignmentTownText(data[4])+")",colour=0x0000FF)
					elif progressType == 2:
						Notify(channel_id,"[**Konsinye**] Avcı ticareti sona erdi!",colour=0x0000FF)
			elif jobType == 2:
				channel_id = QtBind.text(gui,cmbxEvtMessage_consignmentthief)
				if channel_id:
					progressType = data[3]
					if progressType == 0:
						Notify(channel_id,"[**Konsinye**] Hırsız ticareti 10 dakikada başlayacak!",colour=0xFF5722)
					elif progressType == 1:
						Notify(channel_id,"[**Konsinye**] Hırsız ticareti başladı! ("+getConsignmentTownText(data[4])+")",colour=0xFF5722)
					elif progressType == 2:
						Notify(channel_id,"[**Konsinye**] Hırsız ticareti sona erdi!",colour=0xFF5722)
	# SERVER_BA_NOTICE
	elif opcode == 0x34D2:
		...
		if channel_id:
			updateType = data[0]
			if updateType == 2:
				Notify(channel_id,"[**Battle Arena**] ("+getBattleArenaText(data[1])+") 45 dakikada başlayacak!",colour=0xFF0000)
			elif updateType == 13:
				Notify(channel_id,"[**Battle Arena**] ("+getBattleArenaText(data[1])+") 5 dakikada başlayacak!",colour=0xFF0000)
			elif updateType == 14:
				Notify(channel_id,"[**Battle Arena**] ("+getBattleArenaText(data[1])+") 1 dakika sonra başlayacak!",colour=0xFF0000)
			elif updateType == 3:
				Notify(channel_id,"[**Battle Arena**] kayıt dönemi sona erdi!",colour=0xFF0000)
			elif updateType == 4:
				Notify(channel_id,"[**Battle Arena**] başladı!",colour=0xFF0000)
			elif updateType == 5:
				Notify(channel_id,"[**Battle Arena**] sona erdi!",colour=0xFF0000)
	# SERVER_CTF_NOTICE
	elif opcode == 0x34B1:
		...
		if channel_id:
			updateType = data[0]
			if updateType == 2:
				Notify(channel_id,"[**Capture the Flag**] 10 dakikada başlayacak!",colour=0xFF0000)
			elif updateType == 13:
				Notify(channel_id,"[**Capture the Flag**] 5 dakikada başlayacak!",colour=0xFF0000)
			elif updateType == 14:
				Notify(channel_id,"[**Capture the Flag**] 1 dakikada başlayacak!",colour=0xFF0000)
			elif updateType == 3:
				Notify(channel_id,"[**Capture the Flag**] başladı!",colour=0xFF0000)
			elif updateType == 9:
				Notify(channel_id,"[**Capture the Flag**] sona erdi!",colour=0xFF0000)
	# SERVER_QUEST_UPDATE
	elif opcode == 0x30D5:
		...
		if channel_id:
			...
			if data[0] == 2 and data[10] == 2:
				...
				Notify(channel_id,"|`"+character_data['name']+"`| - **Görev** tamamlandı! ***"+quest['name']+"***",colour=0xFF5722)
	...
	# SERVER_FW_NOTICE
		...
		if channel_id:
			updateType = data[0]
			if updateType == 1:
				Notify(channel_id,"[**Kale Savaşı**] 30 dakika içinde başlayacak!",colour=0xFFFF00)
			elif updateType == 2:
				Notify(channel_id,"[**Kale Savaşı**] başladı!",colour=0xFFFF00)
			elif updateType == 3:
				Notify(channel_id,"[**Kale Savaşı**] bitmesine 30 dakika var!",colour=0xFFFF00)
			elif updateType == 4:
				Notify(channel_id,"[**Kale Savaşı**] bitmesine 20 dakika var!",colour=0xFFFF00)
			elif updateType == 5:
				Notify(channel_id,"[**Kale Savaşı**] bitmesine 10 dakika var!",colour=0xFFFF00)
			elif updateType == 8:
				...
				Notify(channel_id,"[**Kale Savaşı**] "+getFortressText(fortressID)+" `"+guildName+"` tarafından alınmıştır! ",colour=0xFFFF00)
			elif updateType == 9:
				Notify(channel_id,"[**Kale Savaşı**] bitmesine 1 dakika var!",colour=0xFFFF00)
			elif updateType == 6:
				Notify(channel_id,"[**Kale Savaşı**] sona erdi!",colour=0xFFFF00)
	# SERVER_PARTY_DATA
	elif opcode == 0x3065:
		party_data = get_party()
		channel_id = QtBind.text(gui_,cmbxEvtParty_joined)
		if channel_id:
			Notify(channel_id,"|`"+character_data['name']+"`| Partiye katıldın!\n"+getPartyTextList(party_data),colour=0x00FFFF)
	# SERVER_PARTY_UPDATE
	elif opcode == 0x3864:
		updateType = data[0]
		if updateType == 1:
			Notify(QtBind.text(gui_,cmbxEvtParty_left),"|`"+character_data['name']+"`| Partiden ayrıldın!",colour=0x00FFFF)
		elif updateType == 2:
			party_data = get_party()
			...
			if channel_id:
				...
				Notify(channel_id,"|`"+character_data['name']+"`| `"+memberName+"` partiye katıldı!\n"+getPartyTextList(party_data),colour=0x00FFFF)
		elif updateType == 3:
			...
			if memberName == character_data['name']:
				...
				if channel_id:
					Notify(channel_id,"|`"+character_data['name']+"`| `"+memberName+"` Partiden ayrıldı!\n"+getPartyTextList(party_data),colour=0x00FFFF)
			...
			if data[5] == 2: # level
				...
				if channel_id:
					...
					if oldLevel < newLevel:
						Notify(channel_id,"|`"+character_data['name']+"`| `"+party_data[joinID]['name']+"` level atladı!\n"+getPartyTextList(party_data),colour=0x00FFFF)
	...
	# SERVER_STALL_ENTITY_ACTION
		...
		if data[0] == 3 and hasStall:
			...
			if channel_id:
				...
				Notify(channel_id,"|`"+character_data['name']+"`| `"+playerName+"` tezgahından bir eşya aldı.\n```Şimdiki Altın > : "+getGoldText()+"```",colour=0x00FFFF)
	# SERVER_CHAT_ITEM_DATA
		...
		for i in range(data[1]):
			...
			try:
				...
				log('Plugin: Öğe ayrıştırma hatası kaydediliyor...')
		...
		if updateType == 6: # member update
			...
			if infoType == 2: # session
				if data[6]:
					...
					if channel_id:
						...
						Notify(channel_id,"|`"+character_data['name']+"`| Lonca üyesi `"+member['name']+"` oturumu kapattı!",colour=0xFF9900)
				else:
					...
					if channel_id:
						...
						if member['name'] != character_data['name']:
							Notify(channel_id,"|`"+character_data['name']+"`|  Lonca üyesi `"+member['name']+"` oturumu açtı!",colour=0xFF9900)
			...
			if infoType == 16: # notice changed
				...
				if channel_id:
					...
					Notify(channel_id,"|`"+character_data['name']+"`| Lonca bildirimi güncellendi! : **`"+title+"`**\n"+text,colour=0xFF9900)
	return True
...
def notify_pickup(channel_id,itemID):
	...
	if not usefilterName and not usefilterServerName:
		...
		genre = getGenreText(item['servername'])
		sox = getSoXText(item['servername'],item['level'])
		Notify(channel_id,"|`"+character_data['name']+"`| - **Öğe** alındı ***"+item['name']+(' '+race if race else '')+(' '+genre if genre else '')+(' '+sox if sox else '')+"***",colour=0xFF9900)
		return
	# check filter name
	if usefilterName:
		...
		if searchName:
			try:
				if re.search(searchName,item['name']):
					...
					genre = getGenreText(item['servername'])
					sox = getSoXText(item['servername'],item['level'])
					Notify(channel_id,"|`"+character_data['name']+"`| - **Öğe(Filtrelenmiş)** alındı! ***"+item['name']+(' '+race if race else '')+(' '+genre if genre else '')+(' '+sox if sox else '')+"***",colour=0xFF9900)
					return
			except Exception as ex:
				log("Plugin: Regex'te hata (ad) ["+str(ex)+"]")
	...
	if usefilterServerName:
		...
		if searchServername:
			try:
				...
				log("Plugin: Regex'te hata (sunucuadı) ["+str(ex)+"]")
...
def on_discord_message(msg,channel_id):
	...
	if msgLower.startswith('get '):
		...
		if msgLower == 'position':
			p = get_position()
			Notify(channel_id,'|`'+character_data['name']+'`| - Gerçek konumunuz ( X:%.1f, Y:%.1f, Region:%d )'%(p['x'],p['y'],p['region']),CreateInfo('position',p))
		elif msgLower == 'party':
			party = get_party()
			if party:
				Notify(channel_id,'|`'+character_data['name']+'`| - Parti üyeleriniz :\n'+getPartyTextList(party))
			else:
				Notify(channel_id,'|`'+character_data['name']+'`| - Partide değilsin!')
		elif msgLower == 'guild':
			guild = get_guild()
			if guild:
				Notify(channel_id,'|`'+character_data['name']+'`| - Lonca üyeleriniz `'+character_data['guild']+'` are :\n'+getGuildTextList(guild))
			else:
				Notify(channel_id,'|`'+character_data['name']+'`| - Sen loncada değilsin!')
		elif msgLower == 'exp':
			data = get_character_data()
			percentExp = data['current_exp'] * 100 / data['max_exp'] 
			Notify(channel_id,'|`'+character_data['name']+'`| - Şu anki deneyiminiz %.2f' % percentExp)

# Plugin loaded
log('Plugin: '+pName+' v'+pVersion+' Yüklendi, Çalışıyor...')

if not os.path.exists(getPath()):
	# Creating configs folder
	os.makedirs(getPath())
	log('Plugin: '+pName+' klasör oluşturuldu!')

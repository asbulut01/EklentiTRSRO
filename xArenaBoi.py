	...
	if OptionsSelected():
		...
		for key, npc in npcs.items():
			if npc['servername'] == r"NPC_BATTLE_ARENA_MANAGER":
				...
				return
		log('Plugin: Arena Sorumlusu yanında olmalısın.')
...
def ConfirmAreaChanged():
	...
	if Setx == 14762.0 and Sety == 2592.0:
		...
		MovetoPick()
		return
	log('Plugin: Kasılma alanı tasınamadı')
	ChangeAreaAttempts = 0
	start_bot()
			...
			for key, player in party.items():
				if player['player_id'] > 0:
					...
					if MembersInside >= WaitFor:
						...
						return
			log('Plugin: Bütün parti üyeleri girmedi..Bekleniyor. !')
...
def ReturntoTraining():
	...
	if QtBind.isChecked(gui,cbxFinished):
		log('Plugin: Kasılma alanına dönüs')
		ClearGUI('Morph',None)
		ClearGUI('Reg',None)
		Started = False
		Attacking = False
	...
	if Started:
		if Registering:
			...
			if not get_party():
				...
				log('Plugin: Parti Üyelerinin İçeri Girmesini Bekleniyor...')
		...
		#successful
			...
			stop_bot()
			QtBind.setText(gui,lblStage,'0')
			log('Plugin: Sonsuzluk Savaşı tüm aşamalar tamamlandı.')
		#party didnt enter
		elif WaitingforParty:
			if PartyCount >= 2:
				Timer(0.1, ReturntoTraining, ()).start()
			log('Plugin: Parti üyesi zamanında girmedi.')
			WaitingforParty = False
			Timer(5.0, StartReg, ()).start()
			Inside = False
			QtBind.setText(gui,lblStage,'0')
	...
	if opcode == 0xB05A and Registering:
		if data[0] == 2 and data[2] == 28:
			...
			if response == 60:
				...
				log("Plugin: Bugünlük tüm aşamaları tamamladın. Yarın tekrar dene!")
				#for solo only
				...
				log("Plugin: Önce Parti Lideri girmeli!")
				if PartyCount >= 2:
				...
				log("Plugin: Parti ile giriş yapamazsın.")
				if QtBind.isChecked(gui,cbxSolo71to80) or QtBind.isChecked(gui,cbxSolo81to90) or QtBind.isChecked(gui,cbxSolo91to100) or QtBind.isChecked(gui,cbxSolo101to110):
					#leave party
...
def SaveConfig():
	...
	log("Plugin: yapılandırmalar kaydedildi")
...
if not os.path.exists(path):
	os.makedirs(path)
	log('Plugin: [%s] Klasör olusturuldu.' % name)

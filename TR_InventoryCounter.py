...
cbxPartyChat = QtBind.createCheckBox(gui,"cbxPartyChat_clicked","Parti Sohbeti", 475, 175)
cbxGuildChat = QtBind.createCheckBox(gui,"cbxGuildChat_clicked","Grup Sohbeti", 475, 190)
cbxUnionChat = QtBind.createCheckBox(gui,"cbxUnionChat_clicked","Birlik Sohbeti", 475, 205)
cbxPrivateChatSender = QtBind.createCheckBox(gui,"cbxPrivateChatSender_clicked","Özel (Gönderene)", 475, 220)
cbxPrivateChatTarget = QtBind.createCheckBox(gui,"cbxPrivateChatTarget_clicked","Özel (Hedefe)", 475, 235)
...
def btnSaveChatSettings_clicked():
    saveConfigs()
    log('TR_InventoryCounter: Sohbet kanalı ayarları manuel olarak kaydedildi.')
...
def send_response(command_sender, message):
    ...
    char_name = "[{}]".format(inGame['name']) if inGame else "[Bot]"
    try:
        if channel == "All":
            phBotChat.All(f"{char_name}: {message}")
        elif channel == "Party":
            if get_party():
                 phBotChat.Party(f"{char_name}: {message}")
            else:
                 phBotChat.Private(command_sender, f"(Party yok) {message}")
                 log("TR_InventoryCounter: Party aktif değil, yanıt özel olarak gönderildi.")
        elif channel == "Guild":
             if get_character_data().get('guild'):
                 phBotChat.Guild(f"{char_name}: {message}")
             else:
                 phBotChat.Private(command_sender, f"(Guild yok) {message}")
                 log("TR_InventoryCounter: Guild aktif değil, yanıt özel olarak gönderildi.")
        elif channel == "Union":
              if get_character_data().get('guild'):
                  phBotChat.Union(f"{char_name}: {message}")
              else:
                  phBotChat.Private(command_sender, f"(Union/Guild yok) {message}")
            ...
            if current_target_name:
                phBotChat.Private(current_target_name, f"{char_name}: {message}")
            else:
                phBotChat.Private(command_sender, "(Hedef karakter adı belirtilmemiş!)")
    ...
    if player and lstLeaders_exist(player) or t == 100:
        if msg == "ENV":
            try:
                ...
                if inventory_data and 'items' in inventory_data and 'size' in inventory_data:
                    size = 0
                    usingSpace = 0

                    items = []
                    items = inventory_data['items'][12:]
                    size = inventory_data['size'] - 12
                    ...
                    size -= 1
                    usingSpace -= 1
                    send_response(player, f"Bos Alan: {size - usingSpace} ----> Dolu: {usingSpace}/{size}")
                else:
                    send_response(player, "Envanter bilgisi alinamadi.")
            except Exception as e:
                log(f"Hata (ENV): {e}")
                send_response(player, "ENV komutu islenirken hata olustu.")
            ...
            try:
                data = get_character_data()
                if data and 'job_current_exp' in data and 'job_max_exp' in data and data['job_max_exp'] > 0:
                    currentExp = data['job_current_exp']
                    maxExp = data['job_max_exp']
                    exp_percentage = (100.0 * currentExp) / maxExp
                    send_response(player, f"Job Exp: %{exp_percentage:.2f}")
                elif data and 'job_level' in data and data['job_level'] == 0:
                    send_response(player, "Karakterin bir meslegi yok.")
                else:
                    send_response(player, "Karakter Job EXP bilgisi alinamadi.")
            except Exception as e:
                log(f"Hata (JOBEXP): {e}")
                send_response(player, "JOBEXP komutu islenirken hata olustu.")
...
log(f'Eklenti: {name} v{version} yüklendi, çalışıyor...')

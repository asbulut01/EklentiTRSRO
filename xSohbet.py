...
cbxMsg = QtBind.createCheckBox(gui, 'cbxMsg_clicked','Mesaj gönder: ', 21, 13)
cbxMsg_checked = False
tbxMsg = QtBind.createLineEdit(gui,"",125,12,480,18)
lblSpamCounter = QtBind.createLabel(gui,"Spam Sayacı :",620,13)
lblCounter = QtBind.createLabel(gui,"0",700,13)

lblLog = QtBind.createLabel(gui,"Artık tüm oyun içi mesajları kaydedebilirsiniz! Kaydetmek istediğiniz sohbet mesajlarının türünü seçmeniz yeterlidir.",21,45)

cbxLogAll = QtBind.createCheckBox(gui,'cbxLog_clicked','Tümü',21,64)
cbxLogPrivate = QtBind.createCheckBox(gui,'cbxLog_clicked','Özel',21,83)
cbxLogParty = QtBind.createCheckBox(gui,'cbxLog_clicked','Parti',21,102)
cbxLogGuild = QtBind.createCheckBox(gui,'cbxLog_clicked','Lonca (Guild)',21,121)
cbxLogUnion = QtBind.createCheckBox(gui,'cbxLog_clicked','Birlik (Union)',21,140)
cbxLogAcademy = QtBind.createCheckBox(gui,'cbxLog_clicked','Akademi',21,159)
cbxLogStall = QtBind.createCheckBox(gui,'cbxLog_clicked','Tezgah',21,178)
cbxLogGlobal = QtBind.createCheckBox(gui,'cbxLog_clicked','Küresel (Global)',21,197)
cbxLogNotice = QtBind.createCheckBox(gui,'cbxLog_clicked','Bildirim (Notice)',21,216)
cbxLogGM = QtBind.createCheckBox(gui,'cbxLog_clicked','GM',21,235)
cbxLogUnknown = QtBind.createCheckBox(gui,'cbxLog_clicked','Diğerleri',21, 254)
cbxLogsInOne = QtBind.createCheckBox(gui, 'cbxLog_clicked','Mesajları tek bir dosyaya kaydet (log.txt)', 21, 286)

cbxEvtSpawn_unique = QtBind.createCheckBox(gui,'cbxLog_clicked','Eşsiz Doğdu!',321, 64)
cbxEvtSpawn_hunter = QtBind.createCheckBox(gui,'cbxLog_clicked','Avcı/Kervancı Doğdu!',321,83)
cbxEvtSpawn_thief = QtBind.createCheckBox(gui,'cbxLog_clicked','Hırsız Doğdu!',321,102)
cbxEvtChar_attacked = QtBind.createCheckBox(gui,'cbxLog_clicked','Karakter Saldırıya Uğradı!',321,121)
cbxEvtChar_died = QtBind.createCheckBox(gui,'cbxLog_clicked','Karakter Öldü!',321,140)
cbxEvtPet_transport_died = QtBind.createCheckBox(gui,'cbxLog_clicked','At/Taşıyıcı Öldü!',321,159)
cbxEvtDrop_item = QtBind.createCheckBox(gui,'cbxLog_clicked','Öğe Düştü!',321,178)
cbxEvtDrop_rare = QtBind.createCheckBox(gui,'cbxLog_clicked','Nadir Öğe Düştü!',321,197)
...
def cbxMsg_clicked(checked):
	...
	if checked:
		...
		log("Plugin: Spam başladı.")
	else:
		log("Plugin: Spam bitirildi.")
...
def chat(args):
	...
	if sent:
		log('Plugin: "'+t+'" Mesaj başarıyla gönderildi!')

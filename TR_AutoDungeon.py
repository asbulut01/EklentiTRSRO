    ...
    if opcode == 0x751A:
        ...
        if itemUsedByPlugin:
            if data[0] == 1:
                ...
                log('Eklenti: "' + itemUsedByPlugin['name'] + '" Açılamadı, 5 Saniye Sonra Tekrar Deneyecek.')
                Timer(5.0, RetryOpenDimensional, [itemUsedByPlugin['name']]).start()
    return True

def RetryOpenDimensional(name):
    log('Eklenti: Taş açılamadı, tekrar denenecek: ' + name)
    GoDimensional([name])  # GoDimensional işlevini tekrar çağırarak taşı açmaya çalışır

...
lblGuildDepo = QtBind.createLabel(gui,'Guild Deposu', 235, 120)
...
def sort_items_inventory():
    ...
    job = get_running_job()
    log('[%s] %s: Başlatıldı.' % (__name__, get_running_job()))
...
def store_gold(type, amount):
    ...
    if amount > 0:
        ...
        while amount >= 10 ** 9:
            if running == False:
                break
            log('[%s] %s: %s Altın DepolaNIYOR.' % (__name__, get_running_job(), 10 ** 9))
            send_store_gold(type, 10 ** 9)
            amount -= 10 ** 9
        if running != False:
            if amount % 10 ** 9:
                log('[%s] %s: %s Altın DepolaNIYOR.' % (__name__, get_running_job(), amount))

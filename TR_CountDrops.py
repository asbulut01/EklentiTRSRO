...
lTITAN = QtBind.createLabel(gui,'EVENT DROPS',40,10)
...
lblCizgi7 = QtBind.createList(gui,555,100,150,90)
...
qweapon = QtBind.createLabel(gui,'0',670,125)
qprotector = QtBind.createLabel(gui,'0',670,140)
qaccessory = QtBind.createLabel(gui,'0',670,155)
qshield = QtBind.createLabel(gui,'0',670,170)

lblCizgi7 = QtBind.createList(gui,555,192,150,100)
lblCizgi10 = QtBind.createLabel(gui,'_________________________',555,197)
lCoins = QtBind.createLabel(gui,'COINS',610,195)
lArenaC = QtBind.createLabel(gui,'Arena Coin',565,212)
lGoldC = QtBind.createLabel(gui,'Gold Coin',565,227)
lSilverC = QtBind.createLabel(gui,'Silver Coin',565,242)
lCopperC = QtBind.createLabel(gui,'Copper Coin',565,257)
lIronC = QtBind.createLabel(gui,'Iron Coin',565,272)
qarena = QtBind.createLabel(gui,'0',670,212)
qgold = QtBind.createLabel(gui,'0',670,227)
qsilver = QtBind.createLabel(gui,'0',670,242)
qcopper = QtBind.createLabel(gui,'0',670,257)
qiron = QtBind.createLabel(gui,'0',670,272)
...
btnPet = QtBind.createButton(gui,'btnPet_clicked'," T. Pet! ",630,29)
...
def countItems(countIn):
    ...
    if items != []:
        for item in items:
            ...
            if item != None and "Monster" in item['name'] and "Summon Scroll" in item['name']:
                MonsterSc += item['quantity']
            if item != None and "Alchemy" in item['name'] and "catalyst" in item['name']:
                Catalyst += item['quantity']
            if item != None and "HP" in item['name'] and "potion" in item['name']:
                HPScroll += item['quantity']
            if item != None and "MP" in item['name'] and "potion" in item['name']:
            ...
            if item != None and "Scroll" in item['name'] and "Evasion" in item['name']:
                sceva += item['quantity']
            if item != None and "Scroll of" in item['name'] and "Accuracy" in item['name']:
                scacc += item['quantity']
            if item != None and "Intensifying" in item['name'] and "(Weapon)" in item['name']:
                weapon += item['quantity']
            if item != None and "Intensifying" in item['name'] and "(Armor)" in item['name']:
                protector += item['quantity']
            if item != None and "Intensifying" in item['name'] and "(Accessory)" in item['name']:
                accessory += item['quantity']
            if item != None and "Intensifying" in item['name'] and "(Shield)" in item['name']:
